function Calendar(options) {
  ...
  const calendarTitle = options.title;
  const fragment = window.document.createDocumentFragment();
  if(calendarTitle){
    fragment.appendChild(buildTitleBar(calendarTitle));
  }
  fragment.appendChild(buildMonthNav());

  const bodyContainer = createElement('div', 'vue-m-bodyContainer');
  bodyContainer.appendChild(buildWeekdays());
  const daysContainer = createElement('div', 'vue-m-days');
  ...
  if(calendarTitle){
    self.cancelSpan.addEventListener('click', cancelCalendar);
    self.blankSpan.addEventListener('click', selectDate); //清除已选的日期
  }
  ...
  function buildTitleBar(title) {
    const titleContainer = createElement('div', 'vue-m-title');
    self.blankSpan = createElement('span' , 'vue-m-title-blank');
    self.blankSpan.innerHTML = '清空';
    const titleSpan = createElement('span' , 'vue-m-title-span');
    titleSpan.innerHTML = title;
    self.cancelSpan = createElement('span' , 'vue-m-title-cancel');
    self.cancelSpan.innerHTML = '取消';

    titleContainer.appendChild(self.blankSpan);
    titleContainer.appendChild(titleSpan);
    titleContainer.appendChild(self.cancelSpan);

    return titleContainer;
  }
  function buildMonthNav() {
    const headContainer = createElement('div', 'vue-m-headContainer');
    const navigationCurrentMonth = createElement('div', 'vue-m-current-month');
    const navigationCurrentYear = createElement('div', 'vue-m-current-year');

    self.currentYearElement = createElement('span', 'cur-year', utils.yearToStr(currentYear));
    self.prevYearNav = createElement('span', 'vue-m-prev-year');
    self.prevYearNav.innerHTML = options.prevArrow;
    self.nextYearNav = createElement('span', 'vue-m-next-year');
    ...
    self.prevMonthNav = createElement('span', 'vue-m-prev-month');
    self.prevMonthNav.innerHTML = options.prevArrow;
    self.nextMonthNav = createElement('span', 'vue-m-next-month');
  ...
  function buildWeekdays() {
    const weekdayContainer = createElement('div', 'vue-m-weekdays');
    ...
    weekdayContainer.innerHTML = `<span class=vue-m-weekday>${weekdays.join('</span><span class=vue-m-weekday>')}</span>`;
  ...
  function createDay(className, date) {
    const dayElement = createElement('span', 'vue-m-day', date.getDate());

    dayElement.dateObj = date;

    if (options.disabledDate && options.disabledDate(date)) {
      toggleClass(dayElement, 'disabled', true);
    } else if (className) {
      toggleClass(dayElement, className, true);
    } else {
  ...
  function selectDate(e) {
    ...
    if (e.target.classList.contains('vue-m-title-blank')) {
      ...
      return
    } else if (!e.target.classList.contains('vue-m-day')) {
      return;
    }
